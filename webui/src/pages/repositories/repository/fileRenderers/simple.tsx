...
export const UnsupportedFileType: FC<RendererComponent> = ({ path, fileExtension, contentType }) => {
    return (
        <Alert variant="warning" className="m-5">
            ...
            <div>{`azlake doesn't know how to render this file (extension = "${fileExtension}", content-type = "${contentType}")`}</div>
        </Alert>
    );
};
...
export const IpynbRenderer: FC<RendererComponentWithText> = ({ text }) => {
    return <Alert variant="info" className="m-5">Notebook rendering is not supported in azlake.</Alert>;
};
...
export const GeoJSONRenderer: FC<RendererComponentWithText> = ({ text }) => {
    return <Alert variant="info" className="m-5">GeoJSON rendering is not supported in azlake.</Alert>;
};
