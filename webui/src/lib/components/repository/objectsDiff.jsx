    ...
    return (
        <div>
            ...
            <span>
                <Alert variant="light">
                    <InfoIcon />{' '}
                    {`azlake supports content diff for ${supportedFileExtensions.join(',')} file formats only`}
                </Alert>
            </span>
        </div>
    );
...
const DiffSizeReport = ({ leftSize, rightSize, diffType }) => {
    ...
    switch (diffType) {
        case 'changed':
            ...
            break;
        case 'conflict': // conflict will compare left and right. further details: https://github.com/treeverse/azlake/issues/3269
            return (
                <div>
